...
学習済みのモデルが用意してありますので、試しに動かしたいという方はこちらを実行してください（GUIが利用可能な環境で実行してください）。（なお正式な利用方法はUser's Manualに掲載しております。）
...
13. python pred_LSTM_ensemble.py を実行する（GUIが利用可能な環境で実行してください）
